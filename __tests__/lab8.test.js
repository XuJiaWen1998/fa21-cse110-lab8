describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const showdow_root = await prodItem.getProperty('shadowRoot');
    const button = await showdow_root.$('button');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    for (let i = 0; i < prodItems.length; i++){
      const showdow_root = await prodItems[i].getProperty('shadowRoot');
      const button = await showdow_root.$('button');
      await button.click();
    }
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const local_storage = await page.evaluate(function(){ 
